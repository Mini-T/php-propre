    ...
    while ($fetchaffi = $affichage->fetch(PDO::FETCH_OBJ)) { ?>
        <div class="card out" style="width: 18rem;">
            ...
            <div class="card-body">
                ...
                if ($fetchaffi->utilisateur == $_SESSION['username']) { ?>
                    <form action="#" method="POST">
                       
                        <input type="hidden" id="hid" name="hid" value="<?php echo $fetchaffi->id ?>"> 
                        <button type="submit" class="btn btn-primary red" name="delete" value=1>Supprimer</button>

                    </form> <?php echo $_POST['hid']; } else { ?>
                        <input type="button" Value="Supprimer" id="disabled" class="btn btn-primary red" disabled>
                   <?php } ?>
            </div>
        </div>
